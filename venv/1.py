...
def spark_analyse(filename):
    ...
    another_list = [0 for i in range (7)]

    another_list[0] = df.filter(df.area == "南岗").agg({"area": "count"}).first()['count(area)']
    another_list[1] = df.filter(df.area == "道里").agg({"area": "count"}).first()['count(area)']
    another_list[2] = df.filter(df.area == "江北").agg({"area": "count"}).first()['count(area)']
    another_list[3] = df.filter(df.area == "香坊").agg({"area": "count"}).first()['count(area)']
    another_list[4] = df.filter(df.area == "道外").agg({"area": "count"}).first()['count(area)']
    another_list[5] = df.filter(df.area == "松北").agg({"area": "count"}).first()['count(area)']
    another_list[6] = df.filter(df.area == "平房").agg({"area": "count"}).first()['count(area)']
    ...
    ans=[]
    ans.append(all_list)
    ans.append(another_list)
    return ans
def draw_bar_first(all_list):
    print("开始绘图1")
    ...
    bar.render("The Bar.html")
    print("结束绘图1")
def draw_bar_second(another_list):
    print("开始绘图2")
    attr = ["南岗", "道里", "江北", "香坊", "道外", "松北", "平房"]
    v = another_list
    pie = Pie("哈尔滨市租房概况")
    pie.add("租房数量", attr, v, is_label_show=True)
    pie.render("The Pie.html")
    print("结束绘图2")
if __name__=='__main__':
    list = spark_analyse(filename)
    all_list = list[0]
    another_list = list[1]
    draw_bar_first(all_list)
    draw_bar_second(another_list)
